...
import { YStack, XStack, SizableText, Button, Spinner, Card, ScrollView } from 'tamagui';
import { X, Camera as CameraIcon, CheckCircle, RefreshCcw } from 'lucide-react-native';
...
export default function ScannerScreen() {
  ...
  if (!permission) {
    return <YStack f={1} jc="center" ai="center"><Spinner size="large" /></YStack>;
  }
  ...
  const findMatchingProduct = (medName: string) => {
    ...
    return storeData.productos.find((p: any) => 
  ...
  const handleAddToCart = (product: any, prescribedUnits: number) => {
    ...
    Alert.alert('¡Agregado!', `Se agregaron ${boxesNeeded} caja(s) de ${product.nombre} al carrito.`);
  };
    ...
    return (
      <YStack f={1} bg="$background" pt="$8">
        <XStack p="$4" jc="space-between" ai="center">
          <SizableText size="$6" fontWeight="bold">Resultados</SizableText>
        ...
        <ScrollView contentContainerStyle={{ padding: 16, gap: 16 }}>
          {results.map((med, index) => {
            ...
            return (
              <Card key={index} bordered p="$4" gap="$2" bg="white">
                <SizableText size="$5" fontWeight="bold" color="$color">Receta: {med.name} {med.dosage}</SizableText>
                <SizableText>Cantidad Recetada: {med.prescribed_units} {med.unit_type}</SizableText>
                
                <YStack mt="$2" p="$3" bg="$backgroundHover" borderRadius="$2" gap="$2">
                  <SizableText size="$3" fontWeight="bold" color="$colorHover">Coincidencia en Tienda:</SizableText>
                  {matchedProduct ? (
                    <>
                      <SizableText>{matchedProduct.nombre}</SizableText>
                      <SizableText color="$colorHover">Bs. {matchedProduct.precio_venta.toFixed(2)}</SizableText>
                      <Button 
                        mt="$2" 
                        bg="$color" 
                        color="white" 
  ...
  if (photoUri) {
    return (
      <YStack f={1} bg="black">
        ...
        {isAnalyzing ? (
          <YStack f={1} jc="center" ai="center" style={StyleSheet.absoluteFillObject} bg="rgba(0,0,0,0.6)">
